...
import com.flearndriving.management.application.entities.Account_;
...
public class AccountSpecification {
    ...
    public static Specification<Account> notEqualUserName(String userName) {
        return (root, query, cb) -> cb.notEqual(root.get(Account_.USER_NAME), userName);
    ...
    public static Specification<Account> likeFullName(String fullName) {
        return (root, query, cb) -> cb.like(
                cb.concat(cb.concat(root.get(Account_.FIRST_NAME), " "), root.get(Account_.LAST_NAME)),
                "%" + fullName + "%");
    }
